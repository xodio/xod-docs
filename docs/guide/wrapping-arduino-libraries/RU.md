...
SoftwareSPI и HardwareI2C. Давайте остановимся на последнем и создадим девайс
для этого типа подключения:
...
void evaluate(Context ctx) {
    ...
    if (!versiondata) {
      // Если плата не ответила своей версией — ошибка подключения или что-то не то с платой
